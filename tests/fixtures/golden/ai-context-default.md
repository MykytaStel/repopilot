...
- Cache: {{CACHE}}
- Graph size: 3 files, 0 import edges

### Edit Order
- framework.django.debug-true in `tests/fixtures`: 1 finding(s), max risk 75
- framework.django.missing-allowed-hosts in `tests/fixtures`: 1 finding(s), max risk 75
- framework.django.raw-sql-query in `tests/fixtures`: 1 finding(s), max risk 37
...
### Verification Focus
- Re-scan the top risky clusters after edits.
