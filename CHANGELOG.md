...
- **The real-repo validation zoo is now a release gate.** `release-contract.py`
  ...
  first so the gate runs for real during release verification. A new
  `docs/engineering/rule-scorecard.md`, generated by `python3 scripts/zoo.py
  scorecard [--write]` from committed zoo expectations and the rules
