...
- Internal: split the 580-line `audits::code_quality::language_risk::pattern` god file by moving each language's AST node-emitter into the sibling pattern module that already owns that language's pattern data (`go`, `js`, `python`, `managed`), leaving only the line/AST dispatch and the shared finding-construction infrastructure (`RiskPattern`, `push_pattern_finding`) in `pattern.rs` (266 lines). Every source file in the module is now under 300 lines. Internal refactor only; no behavior, finding, or public API change.
