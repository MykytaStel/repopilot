...
- **`testing.source-without-test` no longer flags documentation example code or framework entrypoint/config boilerplate.** Files under a `docs/` or `docs_src/` tree are illustrative examples, not production source that warrants a unit test, so they are now skipped alongside the existing `examples/`/`generated/` exclusions. Python framework boilerplate is also exempt: a Django `apps.py` `AppConfig` stub is pure registration (like the already-skipped `settings.py`), and `manage.py`/`wsgi.py`/`asgi.py` are CLI/server entrypoints (the analogue of the already-skipped `__main__.py`). Measured on the real-repo zoo, this took fastapi's strict-profile count from 389 to 36 (its `docs_src/` tutorial tree was 349 of those findings) and wagtail's from 671 to 645, with no change to any default-visible output.
