...
the first additive review projection is now implemented. The follow-up
hardening slice is merged and hosted-validated: unsupported delivery coverage,
skipped verification state, and review-SARIF proof parity are explicit. The
release-candidate/publication gate remains separate and is not claimed here.
...
Unclassified changes inside supported manifest, lockfile, workflow, and
runtime-config paths now remain explicit `unknown` and `limited` contract
deltas; any limited semantic contract keeps the proof at `REVIEW` instead of
allowing a passing check to imply verified coverage.
...
explicitly unavailable. The packet is still exploratory evidence; it does not
claim independent utility or competitor superiority.
...
the added range. The mapping is intentionally narrow; other compiler error
kinds, unchanged-line parser errors, and unsupported baseline failures remain
unavailable until they have an equally exact identity.
