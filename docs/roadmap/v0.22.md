...
[v0.22 repository intelligence design](../engineering/archive/v0.22/v0.22-repository-intelligence-design.md).
...
- The three size and complexity metrics — `long-function`, `complex-function`,
  and `large-file` — were measured last and behaved differently from the
  architecture rules: their findings were factually true. Two calibration gaps
  remained, both about *what* was measured rather than whether the measurement
  was right: vendored and minified artifacts were treated as authored source,
  and `large-file` counted comments as code. With those fixed, all three carry
  sampled evidence at 1.00 precision and zero false-positive debt, so the
  release can state their quality instead of leaving them unmeasured.
...
- Projection-parity audit done across CLI console/JSON/Markdown/HTML/SARIF,
  MCP, and the Action. Five of six surfaces confirmed clean: `repopilot_scan`,
  `repopilot_review_change`, and `repopilot_context` call the same renderer
  functions as the CLI; `repopilot_explain_finding` and
  `repopilot_explain_review_signal` replay the stored record verbatim; scan
  SARIF/HTML/baseline delta and console/markdown filtering all read the same
  `ScanSummary`/`report_stats`/`finding_helpers` functions. One real gap found
  and fixed: review's SARIF renderer re-derived taint severity from
  `signal.kind` string matching instead of the canonical `signal.tier`, which
  could silently under-severity a future `SinkKind` variant.
- The sixth gap is closed. The Action's PR-comment delta used to compute
  new/changed/resolved findings independently in Python
  (`scripts/review_delta.py`) rather than through `src/baseline/diff.rs`. The
  blocker was a real model gap, not a trivial swap: `BaselineScanReport` only
  tagged the *current* scan's findings New/Existing against a saved baseline —
  it had no concept of a finding present in the baseline but absent from the
  current scan, so it could not express "resolved" at all. `BaselineScanReport`
  now carries a `resolved: Vec<BaselineFinding>` field computed the same way
  `New`/`Existing` already were (inverting the same key-or-legacy-descriptor
  match), exposed in JSON/console/markdown/HTML across every `scan --baseline`
  and `review --baseline` consumer, not only the Action. `baseline create`
  gained `--profile`/`--min-severity`/`--min-priority` so the Action's base
  snapshot and head scan apply the same visibility filter — required, because
  a finding one side's filter hides and the other's does not would otherwise
  misreport as spuriously new or resolved. The Action now runs
  `baseline create` on the base revision and `scan --baseline` on the head
  revision directly; `scripts/review_delta.py` and its Python delta model are
  gone. One 3-way distinction did not survive: Python additionally tracked
  "changed" (same stable key, only the line moved), which the canonical model
  already folds into `Existing` by design — `stable_finding_key` deliberately
  excludes the line number so line churn does not spawn a false new+resolved
  pair. The Action's `changed-findings-count` output stays declared for
  compatibility and always reports 0.
