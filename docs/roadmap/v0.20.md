...
| #271 | Done | `release-contract.py` gained `check_zoo_gate()` (runs `python3 scripts/zoo.py scan`, failing the contract on a non-zero exit, when every manifest repo is cloned under `.zoo/`; skips with a note otherwise) and `check_rule_scorecard()` (regenerates the scorecard from committed `tests/zoo/expectations/*.toml` + `docs/rules-reference.md` and fails on staleness or a missing `docs/README.md` link). `verify-release.sh` now runs `python3 scripts/zoo.py clone` before the release-contract step so the gate is exercised for real during release verification. New `scripts/zoo_scorecard.py` + `python3 scripts/zoo.py scorecard [--write]` produce the committed `docs/engineering/rule-scorecard.md` (lifecycle, precision estimate, false-positive debt per rule). 8 new release-contract tests + 6 new scorecard unit tests; `zoo_runner_contract.rs`'s fake-toolchain harness updated to copy the new sibling script. No rule changes. | None. |
| #272 | Done | New `tests/fixtures/review-zoo/<family>/<scenario>/{safe,unsafe}/{before,patch,after}` format (`patch/rationale.md` required+non-empty, documents the exact edit; `before`/`after` reuse the golden-fixture overlay convention) with three families — `boundary/access-control`, `behavioral/network-call`, `taint/sql-injection` — each a minimal-edit safe/unsafe contrast (e.g. taint: same tainted `req.query.id`, safe reaches a safe ORM call, unsafe reaches raw SQL via concatenation). New unconditional `tests/review_zoo.rs` gate (2 tests) asserts `safe/` produces zero `tiered_signals`/`boundary_signals` and `unsafe/` matches its `expected.json`, plus a family-coverage assertion. No review detector changes; `tests/fixtures/review/` and `tests/review_golden_fixtures.rs` untouched. | None. |
| #273 | Done | New `src/findings/occurrence.rs` (`occurrence_key`, hashes exact unmasked evidence — distinct location/snippet always yields a distinct key even when the stable `id` collides), `src/findings/decision.rs` (`DecisionRecord`: severity, confidence, evidence, recommendation, `verification_plan: None` placeholder for #276), and `src/findings/record.rs` (`FindingRecord<'a>`, the shared wrapper) — all computed on demand from `&Finding`, not stored fields, so none of the ~50 `Finding { ... }` construction sites needed touching. Wired into scan/baseline/review JSON (`FindingWithBaselineStatus`/`ReviewJsonFinding` now flatten `FindingRecord`), SARIF (`SarifResultProperties`, both construction sites plus the review-SARIF path via the shared `findings_to_sarif`), AI context JSON (`FindingJson`, schema `1`→`2`), and MCP's `repopilot_explain_finding` (`FindingExplanationReport`) — same `DecisionRecord` type reused verbatim everywhere. Baseline matching (`src/baseline/*`) untouched; new regression test proves two colliding-id findings with different `occurrence_key`s both still resolve to the same baseline entry. Report schema `0.20`→`0.21` additively (`ACCEPTED_SCAN_REPORT_SCHEMA_VERSIONS` keeps `0.16`-`0.20`; new `strict_reader_ignores_occurrence_key_and_decision_fields_on_findings` test proves a new-shape finding still parses as `Finding`). Extended the existing colliding-secret regression tests (`scan_visibility_cli.rs`, `scan_changed_cache_cli.rs`) to assert distinct `occurrence_key`s; re-blessed `ai-context.json`, `scan-json-schema.golden.json`, `scan-sarif.golden.json` goldens (additive-only diffs). 700+ tests green. | None. |
| #274 | Done | New `src/review/impact.rs` (`compute_impact_paths`, `ImpactPaths`/`FileImpact`/`AffectedSurface`) does a depth-bounded BFS over the same `composites::build_importers_by_target` relation the existing one-hop `blast_radius` already uses, splitting each changed file's dependents into `direct_dependents` (hop 1) and `transitive_dependents` (hops 2..depth), plus a rolled-up affected-surface summary (impacted file count, distinct affected directories). Depth is configurable via new `review.impact_path_depth` (`ReviewSection`, default `3`); no new CLI flag. `blast_radius` and the risk overlay that reads it (`apply_blast_radius_overlay`) are completely untouched — impact paths are additive, proven by the existing `review_blast_radius`/`risk_engine` tests staying green unmodified. Wired into `ReviewReport`, console/markdown rendering ("Impact Paths" section), and `ReviewJsonReport` (`impact_paths` + `review.impact_path_depth`/`affected_files`/`affected_directories`); MCP's `repopilot_review_change` inherits it via the shared JSON. Report schema `0.21`→`0.22` additively. New `tests/review_impact_paths.rs` (3 e2e tests: depth split, depth-1 override, determinism + empty-for-leaf-change) plus 6 unit tests in `impact.rs`. 89 test binaries green. | None. |
| #275 | Done | New `src/review/signal_pass.rs` (`detect_review_signals`) replaces the two disconnected top-level passes (`detect_boundary_signals` + `detect_content_signals`) with one loop over `changed_files` that builds each side's `ReviewSource` at most once and shares it across boundary's AST fallback, behavioral, algorithmic, and taint-lite. `ReviewSource` (`signals/content.rs`) now memoizes its own tree-sitter parse via `tree()` (was: every detector called `.parsed()`, constructing a fresh `ParsedFile` — and thus re-parsing — every time), cutting a fully-toggled file from up to 3 reads/7 parses to at most 2/2. `classify_boundary_ast_from_source` added alongside the existing `classify_boundary_ast` (kept `#[cfg(test)]`-only, still exercised by its ~10 isolated unit tests) so both entry points share one `walk_ast_for_boundary` implementation. No detector's matching logic changed. `behavioral.rs` module doc gained a "flag, don't prove" limitations paragraph matching boundary/algorithmic/taint's existing style. New `tests/review_signal_pass.rs` proves one review call surfaces all four delta types from a single changed file; existing `review_boundary_signals`/`review_tiered_signals`/`review_golden_fixtures`/`review_zoo` regression suites pass unmodified. 90 test binaries green. | None. |
| #276 | Done | New `src/findings/verification.rs` (`build_verification_plan`) populates the `DecisionRecord.verification_plan` placeholder from #273: `None` unless `finding.confidence == Confidence::High`; otherwise one step per `Evidence` entry (`file:line[-line_end]` + snippet), one category-specific "what to check" step (Security/Architecture/CodeQuality/Testing/Framework — no new rule-authoring metadata needed), and a fixed closing step stating the plan is static-evidence-only, not proof. Pure function of `&Finding` — no LLM, no randomness, deterministic by construction. Wired into `build_decision_record` (`src/findings/decision.rs`, one line), so every `DecisionRecord` consumer inherits it for free: scan/baseline/review JSON, SARIF, AI-context JSON, MCP `repopilot_explain_finding`. Console/Markdown/HTML findings renderers (which work directly off `&Finding`, bypassing `DecisionRecord`) each render a `Verification:` block when present. Scope note: covers `Finding`/`DecisionRecord` only, not review's `ReviewSignal`/`tiered_signals` (boundary/behavioral/algorithmic/taint deltas never become `Finding`s) — confirmed with user as out of scope. New `tests/finding_verification_plan.rs` (presence/absence/determinism via real `scan --format json`); re-blessed `ai-context.json`, `scan-console-full.txt`, `scan-markdown.md`, `scan-sarif.golden.json`, `scan-json-schema.golden.json` goldens (additive-only diffs, confirmed via `git diff`). Self-scan: 242/242 high-confidence findings got a plan, 0/109 non-high-confidence findings did. 91 test binaries green. | None. |
| #277 | Done | Every MCP tool result now includes `workspaceRevision`; scan/review also return a content-derived `analysisHandle`, with the eight most recent full reports retained for handle-based `repopilot_explain_finding` and revision-anchored `repopilot_context`. Stale/expired handles fail explicitly. Scan/review accept `offset`/`limit` and return `pagination` (`total`, `returned`, `next_offset`) without changing report schemas; review handles retain the full report even when the client requests compact output. `repopilot mcp --max-response-bytes` bounds serialized tool results at a configurable 1 MiB default. Non-Git revisions now hash workspace file content while excluding RepoPilot cache files. Coverage: 46 MCP unit tests, 5 session revision tests, and 7 `mcp_cli` integration tests. | Proceed to #278 canonical analysis artifact. |
...
| #281 | In progress | Added shared `RootConfinement` for MCP tool arguments and finding replay, including missing-path lexical traversal, existing-path canonicalization, macOS path aliases, and symlink-escape rejection. Added one human-output evidence-redaction policy for secret/private-key findings across console, Markdown, HTML, AI context, review renderers, and rendered verification steps while leaving JSON/SARIF finding evidence contracts unchanged. Coverage: `path_security` unit tests, MCP/explain regressions, renderer tests, and updated human-output goldens. | Extend the shared boundary to remaining path consumers, verify cache recovery/determinism and complete platform smoke evidence. |
