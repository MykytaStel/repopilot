...
Status: release-candidate; Phase 0 is closed for 0.23.0 except 0C, which closes
with the tag's hosted publication verification (RP23-001).
...
The local audit exposes three required evidence tracks for this baseline:
real-history adjudication, differential utility, and rule-quality coverage.
The rule-quality track checks the committed scorecard and its
snapshot/expectation denominators, reports measured and unmeasured rules, and
cannot close Phase 0 while coverage or held-out recall remains incomplete.
...
- [x] Update the changelog and ledger with precise closure evidence, run the
  handoff gate, and open one focused PR. RP23-003 remained open at the 0B1
  handoff.
...
- Released producer to current scan reader: cover every distinct schema emitted
  by release tags v0.16.0 through v0.22.0 with checksum-verified GitHub release
  assets, version-provenanced report fixtures, and current-parser tests.
- Released Rust reader to schema 0.26: probe exact crates.io versions 0.20.0,
  0.21.0, and 0.22.0 through the public `api::report` facade. Expected schema
  rejection is an explicit supported outcome, not a test failure.
- Baseline: consume the schema-1 baseline emitted byte-identically by v0.20.0
  and v0.22.0, including a line-movement regression that must remain existing,
  never new plus resolved.
- History: preserve incompatible receipts, produce no risk delta, and emit a
  warning that names the comparison reason. Distinct occurrence identities must
  continue to compare independently even when baseline IDs match.
- SARIF is versioned by SARIF 2.1.0 and is not scan-reader input. Action and MCP
  project the canonical reports but do not ingest stored scan JSON. Record these
  surfaces as not applicable for this matrix and retain their existing tests.
- Released tags do not exist for every accepted transition schema. Keep 0.16,
  0.17, 0.21, 0.22, 0.24, and 0.25 labeled synthetic current-reader fixtures;
  do not invent released-producer provenance for them.

Acceptance: committed evidence records tag SHAs, release-asset SHA-256 values,
crates.io checksums, report hashes, platform, commands, and limitations. The
current reader accepts all released-producer fixtures. Older readers reject or
accept schema 0.26 exactly as recorded. Incompatible history is visible and
cannot produce fake resolved findings. No network access enters runtime scans.

Execution: [0B2 implementation plan](v0.23-phase-0b-compatibility-plan.md).

### 0C — Recoverable Publication

Status: release-candidate; implementation merged. Closes with the v0.23.0 tag's
hosted publication and supported-platform installation evidence (RP23-001).
...
Status: verified; implementation merged. Documentation parity guards,
bounded-scope copy, the release closure scope, and the current-state ledger
reconciliation (2026-09-24) are in place.
