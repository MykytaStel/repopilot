...
Status: in-progress; implementation merged. Hosted publication and
supported-platform installation evidence is still required for closure.
...
Status: in-progress; implementation merged. Documentation
parity guards and bounded-scope copy are present, while the historical ledger
and release scorecard still require current-state reconciliation.
