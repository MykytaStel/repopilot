...
> **Historical pre-release snapshot.** This checklist records the state before
> the `v0.22.0` tag and publication. Its unchecked tag, platform, and
> checksum rows are historical observations, not the current distribution
> status. See the [current v0.23 evidence ledger](../../v0.23-evidence-ledger.md)
> for fresh release-channel and installation evidence.

Use this checklist as the archived record for the 0.22 release cycle. Each gate is either
...
## Historical pre-release blockers

The following section is retained as an audit trail. It describes what was
blocking the release cut at the time of this snapshot and must not be read as
the current state of the already-published release.
