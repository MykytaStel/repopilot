...
| RP23-015 | P1 defect | in-progress | Proof UX / A–D | Dogfood 2026-09-24: default console output printed three top-level conclusions (`Decision:`, `Change Proof:`, `Legacy merge readiness:`) and repeated the same why/limitation text. Fixed on `fix/v023-first-screen-truth`: the first screen is `Decision: X (Change Proof: Y)`, reasons, and one next action; provenance, legacy readiness, ownership, and location move to `--detail full`; Markdown uses a separate "Proof details" block. Pinned by `tests/review_first_screen.rs` and the updated projection-parity test. Closes when the branch merges. |
| RP23-016 | P1 defect | in-progress | Proof coverage / A | `known_excluded_files` counted low-signal skipped files — changed **tests**, fixtures, examples, and generated files — as `excluded`, so a change that added tests reported limited coverage and the next action "Review the excluded or unsupported files". Fixed on `fix/v023-first-screen-truth`: they are reported as additive `policy_skipped_files` and no longer reduce coverage; size-limit, binary, file-limit, and `.repopilotignore` exclusions still do. Pinned by `tests/review_first_screen.rs` and `proof_tests.rs`. Closes when the branch merges. |
| RP23-017 | P2 defect | in-progress | Provenance / A | `unavailable_inputs` always listed current/head/base revision as unavailable. Fixed on `fix/v023-first-screen-truth`: reviews resolve and record `base_commit`/`head_commit`, and only uncaptured inputs stay unavailable (working-tree heads are identified by the workspace revision). Pinned by `evidence_tests.rs` and `tests/review_first_screen.rs`. Closes when the branch merges. |
| RP23-018 | P2 UX defect | in-progress | Onboarding / C–D | Without a proof policy the next action put exclusions ahead of policy setup. Fixed on `fix/v023-first-screen-truth`: human-review evidence first, then a missing policy names `repopilot init --suggestions-output`, then coverage limits. An end-to-end test proves a configured passing check reaches `VERIFIED`. Closes when the branch merges. |
| RP23-019 | P2 noise | in-progress | Diagnostics / D | The default first screen listed informational diagnostics such as `analysis.unresolved-local-import-limited`. Fixed on `fix/v023-first-screen-truth`: compact output keeps warnings/errors and folds informational diagnostics into one count; `--detail full` and JSON keep the full list. Closes when the branch merges. |
| RP23-020 | P3 debt | in-progress | Rendering / D | The 166-line console header and 218-line Markdown summary functions were split into `render/console/header.rs` and `render/markdown/summary.rs` while fixing RP23-015 (console.rs 516→350 lines, markdown.rs 483→313). Closes when the branch merges. |
| RP23-021 | P2 evidence-gap | in-progress | Phase D fixtures | Added on `fix/v023-first-screen-truth`: review no-color output (no ANSI escapes with `NO_COLOR`), HTML accessibility landmarks (`lang`, one `h1`, labelled Proof Card), and Windows-separator evidence identity. Narrow terminals are **accepted**: review console output is line-oriented with no width-dependent layout or truncation, so there is no width behavior to pin. Closes when the branch merges. |
| RP23-022 | P2 process | open | Release contract / E | `check_v023_docs()` pins lifecycle markers such as `Status: Phase 0 implementation in progress` and 0C/0D `in-progress`. The version cut must update the markers and contract together so the released docs do not claim work is still in progress. |
