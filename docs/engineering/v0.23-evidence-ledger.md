...
### B14.3 — Experimental Exact Rule Signal (2026-09-20)

An isolated, local tuning case exercised the rule-level mutation path with a
supported `scan` rule. The pinned Express copy changed
`require('./application')` to the missing explicit local target
`require('./missing.js')`; the independent Node oracle failed as expected. The
case declared `expected_rule_ids = ["architecture.unresolved-local-import"]`
and used `analysis_mode = "changed"`.

The baseline scan was measured, the mutated scan introduced exactly one
`architecture.unresolved-local-import` finding, and the reverse patch passed.
The exact violation signal and expected-rule match were both `1/1` with a
positive RSS sample of 19,216 KiB. The manifest, summary, metrics, and report
are local experimental artifacts under
`.zoo/repopilot-validation/experiments/unresolved-local-import/`; the summary
SHA-256 is
`6fe7f9f459bbfa041c4606ce0dd9a5de3dc8f7af44750e816b387dcdc53f511d` and the
metrics SHA-256 is
`35116278fbdb622027913200788ff638a091dd8ad6b12a2d563a8d179278f654`.

This is one tuning case without a negative control or held-out evaluation
population. It validates the runner's exact-rule comparison for this supported
rule family; it is not production recall, precision, additional value, or a
reason to relabel the committed zoo scorecard.
