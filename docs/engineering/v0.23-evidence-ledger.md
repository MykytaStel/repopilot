...
| RP23-012 | P2 design-risk | in-progress | Proof semantics / A | The merged foundation derives path-aware role-scoped obligations from visible sensitive signals, supported contract deltas, and configured/selected checks. Post-merge audit found three parity gaps: unsupported delivery deltas could appear assessed, review SARIF omitted canonical proof, and selected skipped checks could be mislabeled unselected. This follow-up adds blocking limited coverage, gated SARIF proof/verification projection, and distinct unavailable/stale skip states; close after merge and hosted CI confirmation. |
...
- Boundary: local tests and static gates are evidence for this branch only;
  the follow-up is not merged or hosted-validated yet. Publication recovery
  (0C), documentation reconciliation (0D), rule quality (E), and independent
