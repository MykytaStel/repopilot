...
- RP23-001 remains `in-progress`: exact GitHub, npm, and Homebrew inspection
  plus one macOS arm64 installation smoke now have fresh evidence, but the
  complete registry/platform matrix is not yet closed.
- RP23-005 remains `in-progress`: release-contract guards and current-scope
  copy are present, but historical scorecard and lifecycle claims still need
  reconciliation against the current main revision and fresh evidence.
