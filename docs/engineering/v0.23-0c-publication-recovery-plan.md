...
Status: implementation merged in PR 3; exact-version installation support and
Homebrew digest parity are now covered in this follow-up. Hosted publication
and installation evidence remain pending.
...
- [x] Verify all four Homebrew formula archive digests against the exact
  release checksums during post-publication convergence.
- [x] Allow `install.sh` to pin `REPOPILOT_VERSION` so a release candidate can
  run an exact-version checksum/install smoke without querying mutable `latest`.
